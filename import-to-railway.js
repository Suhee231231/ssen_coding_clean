    ...
    try {
        ...
        for (const subject of exportedData.subjects) {
            await railwayConnection.execute(
                'INSERT INTO subjects (id, name, description, is_public, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [subject.id, subject.name, subject.description, subject.is_public, subject.sort_order, subject.created_at, subject.updated_at]
            );
        }
        ...
        for (const problem of exportedData.problems) {
            await railwayConnection.execute(
                'INSERT INTO problems (id, title, content, option_a, option_b, option_c, option_d, correct_answer, explanation, difficulty, category, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                [problem.id, problem.title, problem.content, problem.option_a, problem.option_b, problem.option_c, problem.option_d, problem.correct_answer, problem.explanation, problem.difficulty, problem.category, problem.created_at, problem.updated_at]
            );
        }
